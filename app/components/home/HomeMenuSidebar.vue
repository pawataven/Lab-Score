...
<template>
  <div class="bg-white rounded-xl border border-gray-100 p-4 pb-[30%] shadow-sm">
    ...
    <div class="relative mb-4">
      ...
      <svg class="w-4 h-4 text-gray-500 absolute left-3 top-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="space-y-1">
      <label v-for="league in leagues" :key="league.id" :class="[
        ...
        <div class="flex items-center space-x-2 text-gray-700 overflow-hidden">
          <div v-if="!league.logo" class="w-6 h-6 rounded-full bg-gray-200 shrink-0"></div>

          <NuxtImg v-else :src="league.logo" format="webp" quality="80" loading="lazy"
            class="w-6 h-6 object-contain shrink-0 " :alt="league.name" />
